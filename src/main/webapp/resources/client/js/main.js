(function ($) {
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const prevSaleButton = document.querySelector('.carousel-button.prevSale');
        const nextSaleButton = document.querySelector('.carousel-button.nextSale');
        const saleProductList = document.querySelector('.list-sale-product');

        let scrollAmountSale = 0;
        const scrollStepSale = 200; // Số pixel di chuyển mỗi lần bấm nút

        prevSaleButton.addEventListener('click', () => {
            scrollAmountSale -= scrollStepSale;
            if (scrollAmountSale < 0) scrollAmountSale = 0;
            saleProductList.scrollTo({
                left: scrollAmountSale,
                behavior: 'smooth'
            });
        });

        nextSaleButton.addEventListener('click', () => {
            scrollAmountSale += scrollStepSale;
            if (scrollAmountSale > saleProductList.scrollWidth - saleProductList.clientWidth) {
                scrollAmountSale = saleProductList.scrollWidth - saleProductList.clientWidth;
            }
            saleProductList.scrollTo({
                left: scrollAmountSale,
                behavior: 'smooth'
